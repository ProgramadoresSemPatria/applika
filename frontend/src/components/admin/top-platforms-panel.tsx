...
import { useTopPlatforms } from "@/hooks/use-admin";
import { Skeleton } from "@/components/ui/skeleton";
import { Card } from "@/components/ui/card";

export function TopPlatformsPanel() {
  const { data, isLoading } = useTopPlatforms();
    ...
    >
      <Card className="overflow-hidden p-5">
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {data?.map((platform, i) => {
              const pct = (platform.total_across_users / max) * 100;
                ...
                >
                  ...
                  <div className="h-1.5 overflow-hidden rounded-full bg-accent/50">
                    <motion.div
                      ...
                      style={{
                        background: `linear-gradient(90deg, hsl(var(--primary)) 0%, #fbbf24 100%)`,
                      }}
                      initial={{ width: 0 }}
                      animate={{ width: `${pct}%` }}
                      transition={{
