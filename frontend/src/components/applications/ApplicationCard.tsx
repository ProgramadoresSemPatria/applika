...
export default function ApplicationCard({
  ...
  const steps: Step[] = [
    { id: '1', step_id: 's1', step_name: 'Initial Interview', step_date: '2025-09-20', observation: 'Strong communication.', step_color: '#4ade80' },
  ];
    ...
    <div className="transition-all duration-300 w-full rounded-2xl p-8 my-1 bg-white/5 border border-white/20 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.1)] hover:shadow-[0_6px_20px_rgba(0,0,0,0.15)] hover:-translate-y-0.5">
      ...
      <div className="flex justify-between items-center gap-4 min-h-[45px]">
        ...
        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 items-center min-w-0 text-center">
          {/* similar badge rendering */}
        </div>
        {/* actions */}
        <div className="flex items-center justify-end gap-2 min-w-[80px] text-white/80">
          <button onClick={() => onAddStep(app)} className="text-sky-500 hover:text-sky-600 transition-transform duration-300 hover:scale-110"><i className="fa-solid fa-plus" /></button>
          <button onClick={() => onFinalizeApp(app)} className="text-amber-500 hover:text-amber-600 transition-transform duration-300 hover:scale-110"><i className="fa-solid fa-flag-checkered" /></button>
          <button onClick={() => onEditApp(app)} className="hover:text-white transition-transform duration-300 hover:scale-110"><i className="fa-solid fa-pen-to-square" /></button>
          <button onClick={() => onDeleteApp(app)} className="text-red-500 hover:text-red-600 transition-transform duration-300 hover:scale-110"><i className="fa-solid fa-trash" /></button>
      ...
      <CardDetails
        ...
        onDeleteStep={step => onDeleteStep(step, steps)}
      />
    </div>
  );
}
