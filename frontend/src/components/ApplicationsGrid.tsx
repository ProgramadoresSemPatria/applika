...
export default function ApplicationsGrid({ applications }: ApplicationsGridProps) {
  ...
  const toggleDetails = (id: string) => setOpenCardId(prev => (prev === id ? null : id));
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      {applications.map(app => {
        ...
        return (
          <div key={app.id} className="transition-all duration-300 w-full rounded-2xl p-8 my-1 bg-white/5 border border-white/20 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.1)] hover:shadow-[0_6px_20px_rgba(0,0,0,0.15)] hover:-translate-y-0.5">
            {/* Card Header & Info */}
            <div className="flex justify-between items-center gap-4 min-h-[45px]">
              ...
              <div className="flex items-center justify-end gap-2 min-w-[80px] text-white/80">
                <button onClick={() => handleAddStepClick(app)} className="text-sky-500 hover:text-sky-600 transition-transform duration-300 hover:scale-110">
                  <i className="fa-solid fa-plus" />
                </button>
                <button onClick={() => handleFinalizeClick(app)} className="text-amber-500 hover:text-amber-600 transition-transform duration-300 hover:scale-110">
                  <i className="fa-solid fa-flag-checkered" />
                </button>
                <button onClick={() => handleEditClick(app)} className="hover:text-white transition-transform duration-300 hover:scale-110">
                  <i className="fa-solid fa-pen-to-square" />
                </button>
                {/* Delete */}
                <button onClick={() => handleDeleteClick(app)} className="text-red-500 hover:text-red-600 transition-transform duration-300 hover:scale-110">
                  <i className="fa-solid fa-trash" />
                </button>
                <button onClick={() => toggleDetails(app.id)} className="text-white hover:text-white/80 transition-transform duration-300 hover:scale-110">
            ...
            <CardDetails
              ...
              steps={[{ id: '1', step_id: 's1', step_name: 'Initial Interview', step_date: '2025-09-20', observation: 'Strong communication.', step_color: '#4ade80' }]}
            />
          </div>
        );
      })}
      ...
      <AddStepModal
        ...
        steps={[{ id: 's1', name: 'Initial Interview' }, { id: 's2', name: 'Technical Test' }, { id: 's3', name: 'HR Interview' }]}
        applicationInfo={selectedApplication ? `${selectedApplication.company} - ${selectedApplication.role}` : ''}
      ...
      <FinalizeApplicationModal
        ...
        feedbacks={[{ id: 'f1', name: 'Positive' }, { id: 'f2', name: 'Neutral' }, { id: 'f3', name: 'Negative' }]}
      ...
      <EditApplicationModal
        ...
        platforms={[{ id: 'p1', name: 'LinkedIn' }, { id: 'p2', name: 'Indeed' }, { id: 'p3', name: 'Glassdoor' }]}
