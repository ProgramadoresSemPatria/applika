...
type ApplicationAnalyticsDashboardProps = {
  stats: ApplicationsStatistics;
};

export default function ApplicationAnalyticsDashboardClientUI({ stats }: ApplicationAnalyticsDashboardProps) {
  const cards = [
      ...
      <div className="dashboard-card-size backdrop-blur-[20px] bg-white/5 border border-white/20 rounded-2xl p-8 my-4 shadow-[0_8px_32px_rgba(0,0,0,0.1)]">
        <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-4 mb-4">
          {cards.map(({ label, value }) => (
            <div
              key={label}
              className="bg-white/10 p-6 rounded-xl text-center backdrop-blur-[10px] border border-white/20 transition-all duration-300 ease-in-out hover:bg-white/15 hover:-translate-y-0.5"
            >
